...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--count', type=int, default=10, help='Number of books to create (default: 10)')

    def handle(self, *args, **options):
        count = options['count']
        fake = Faker()
        
        self.stdout.write(self.style.SUCCESS(f'Starting to create {count} dummy books...'))
        ...
        for i in range(count):
            ...
            book = Book.objects.create(
                ...
                cover_image=f"https://picsum.photos/id/{random.randint(1, 1000)}/200/300" if random.choice([True, False]) else None,
                language=language,
                genre=genre,
